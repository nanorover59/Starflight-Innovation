...
public class GuideBookScreen extends Screen
{
    ...
    public void render(MatrixStack matrices, int mouseX, int mouseY, float delta)
	{
        ...
        int x = (this.width - WIDTH) / 2 - 28;
        int y = (this.height - HEIGHT) / 2 + 16;
        DrawableHelper.drawTexture(matrices, x, y, 0, 0, WIDTH, 180, WIDTH, HEIGHT);
        DrawableHelper.drawTexture(matrices, x + 54, y, 0, 0, WIDTH, 180, -WIDTH, HEIGHT);
	        ...
	        for(int i : section.itemIconList.keySet())
			{
	        	if(i >= pageIndex * PAGE_LENGTH * 2 && (i - pageIndex * PAGE_LENGTH * 2) / (PAGE_LENGTH * 2) < pageIndex + 1)
	        	{
	        		...
	        		for(ItemStack itemStack : stacks)
	                	itemRenderer.renderInGuiWithOverrides(itemStack, ((i % (PAGE_LENGTH * 2)) < PAGE_LENGTH ? (x + PAGE_LENGTH) : (x + 172)) + ((inline++ * iconWidth) / 2), y + PAGE_LENGTH + (i % PAGE_LENGTH) * this.textRenderer.fontHeight);
	        	}
			}
        }
        else
        {
        	DrawableHelper.drawTexture(matrices, x + 32, y + 56, 0, 192, 96, 64, WIDTH, HEIGHT);
        }
