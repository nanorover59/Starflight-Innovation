...
public class MetallicScorpionEntity extends TameableEntity implements Angerable, AlienMobEntity
{
	private static final TrackedData<Integer> ANGER_TIME = DataTracker.registerData(MetallicScorpionEntity.class, TrackedDataHandlerRegistry.INTEGER);
	private static final UniformIntProvider ANGER_TIME_RANGE = TimeHelper.betweenSeconds(30, 60);
	@Nullable
	private UUID angryAt;

	public MetallicScorpionEntity(EntityType<? extends TameableEntity> entityType, World world)
	{
	...
	protected void initGoals()
	{
		...
		this.targetSelector.add(5, new UniversalAngerGoal<MetallicScorpionEntity>(this, true));
	}

	public static DefaultAttributeContainer.Builder createMetallicScorpionAttributes()
	{
		return MobEntity.createMobAttributes().add(EntityAttributes.GENERIC_MOVEMENT_SPEED, 0.3f).add(EntityAttributes.GENERIC_MAX_HEALTH, 40.0).add(EntityAttributes.GENERIC_ATTACK_DAMAGE, 4.0);
	}
	...
	public boolean isFireImmune()
	{
		...
		return !AirUtil.canEntityBreathe(this, data);
	}
	...
	{
		MetallicScorpionEntity seleniteEntity = StarflightEntities.METALLIC_SCORPION.create(serverWorld);
	...
	{
		if(other == this || !this.isTamed() || !(other instanceof MetallicScorpionEntity))
			return false;
		
		MetallicScorpionEntity seleniteEntity = (MetallicScorpionEntity) other;
