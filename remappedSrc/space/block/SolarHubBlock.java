...
public class SolarHubBlock extends BlockWithEntity implements EnergyBlock
{
	...
	public double getPowerOutput(World world, BlockPos pos, BlockState state)
	{
		...
		else
		{
			...
			if(world.isClient())
			{
				ClientPlanet planet = ClientPlanetList.getViewpointPlanet();
				
				if(planet != null)
					solarMultiplier = planet.getSolarMultiplier() * (1.0f - ((planet.hasWeather && !ClientPlanetList.isViewpointInOrbit()) ? world.getRainGradient(1.0f) : 0.0f));
				else
