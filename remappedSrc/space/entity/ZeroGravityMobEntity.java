...
public class ZeroGravityMobEntity extends MobEntity
{
	...
	class RandomFlightGoal extends Goal
	{
		...
		public void start()
		{
			boolean upBias = getY() < method_48926().getTopPosition(Type.WORLD_SURFACE, getBlockPos()).getY() + 8;
	...
	class TrackTargetGoal extends Goal
	{
		...
		public boolean canStart()
		{
			return goalSelector.getRunningGoals().count() == 0 && getTarget() != null && getVelocity().length() < 0.1 && getY() > method_48926().getTopPosition(Type.WORLD_SURFACE, getBlockPos()).getY() + 8;
		}
