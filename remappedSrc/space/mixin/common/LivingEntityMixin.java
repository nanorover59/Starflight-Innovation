	...
	public void baseTickInject(CallbackInfo info)
	{
		...
		if(data != null && data.overridePhysics())
		{
			...
			else
			{
				...
				airMultiplier = (float) AirUtil.getAirResistanceMultiplier(this.method_48926(), data, this.getBlockPos());
			}
		}
		else
		{
		...
		if(gravity < 0.01)
		{
			...
			for(Direction direction : Direction.values())
			{
				...
				if(this.method_48926().getBlockState(offset).isAir())
				{
					wall = true;
					break;
				}
		...
		{
			LivingEntity thisEntity = (LivingEntity) this.method_48926().getEntityById(getId());
			...
			if(thisEntity != null)
			{
				...
				if(spaceSuitCheck == 4)
				{
					...
					if(thisEntity.isSneaking() && !creativeFlying && data.isOrbit())
					{
						...
						StarflightEffects.sendJet(this.method_48926(), thisEntity.getPos().add(0.0, 0.65, 0.0), thisEntity.getVelocity().add(thisEntity.getRotationVector().multiply(-2.0)));
					}
				...
				if(gravity < 0.5 && thisEntity instanceof MobEntity && !thisEntity.hasNoGravity() && thisEntity.isOnGround())
				{
					...
					else
					{
						if(thisEntity.horizontalSpeed > 0.25)
						{
							...
							VoxelShape shape = this.method_48926().getBlockState(pos).getCollisionShape(this.method_48926(), pos);
							
							if(!shape.isEmpty() && shape.getBoundingBox().getLengthY() > thisEntity.getStepHeight() && !this.method_48926().getBlockState(pos.up((int) (1.0 / gravity))).blocksMovement())
							{
